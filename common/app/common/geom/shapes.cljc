...
(defn move
  ...
  (let [dx (d/check-num dx)
        dy (d/check-num dy)]
    (-> shape
        (assoc-in [:modifiers :displacment] (gmt/scale-matrix dx dy))
        (gtr/transform-shape)))
...
(defn selection-rect
  ...
  [shapes]
  (let [minx   (transduce (map (comp :x1 :selrect)) min ##Inf shapes)
        miny   (transduce (map (comp :y1 :selrect)) min ##Inf shapes)
        maxx   (transduce (map (comp :x2 :selrect)) max ##-Inf shapes)
        maxy   (transduce (map (comp :y2 :selrect)) max ##-Inf shapes)]
    {:x1     minx
     :y1     miny
     :x2     maxx
     :y2     maxy
...
;; EXPORTS
(defn center [shape] (gco/center shape))
